...
class TestServices(TestUtilities, unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.config['driver'] == "firefox":
            ...
            cls.base_driver = webdriver.Firefox(options=options,
                                                capabilities=capabilities,
                                                service_log_path='/tmp/geckodriver.log',
                                                firefox_profile=profile)
            cls.second_driver = webdriver.Firefox(options=options,
                                                  capabilities=capabilities,
                                                  service_log_path='/tmp/geckodriver.log',
                                                  firefox_profile=profile)
    ...
    def test_topology_graph(self):
        ...
        # Test before fetch update
        self.get_resource(label, '/admin/topology/topology/', 'field-label')
        self.base_driver.execute_script('return document.readyState;')
        self.base_driver.find_elements_by_class_name('visualizelink')[0].click()
        self.assertNotEqual(-1, self.base_driver.page_source.find('<b>nodes</b>: <span>0</span>'))
        # Test after fetch update
        self.base_driver.get(self.config['app_url'] + '/admin/topology/topology')
        self.select_resource(label)
        self.base_driver.find_element_by_name('action') \
                        .find_element_by_xpath('//option[@value="update_selected"]') \
                        .click()
        self.base_driver.find_element_by_name('index').click()
        # import pdb; pdb.set_trace()
        self.get_resource(label, '/admin/topology/topology/', 'field-label')
        self.base_driver.execute_script('return document.readyState;')
        self.base_driver.find_elements_by_class_name('visualizelink')[0].click()
        self.assertNotEqual(-1, self.base_driver.page_source.find('<b>nodes</b>: <span>23</span>'))


    def test_admin_login(self):
        self.login()
        self.login(self.second_driver)
        try:
    ...
    def test_websocket_marker(self):
        ...
        self.get_resource(location_name, '/admin/geo/location/', driver=self.second_driver)
